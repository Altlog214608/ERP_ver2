...
class Sales_Performance(tk.Frame):
    ...
    def create_order_form(self): # 등록
        ...
        self.label15 = ttk.Label(self.topleft_Frame, text="현 재고",width=w, anchor="w")
        self.label15.grid(row=3, column=2, padx=5, pady=5, sticky="w")
        self.entry15 = ttk.Entry(self.topleft_Frame, width=e,state="disabled")
        self.entry15.grid(row=3, column=3, padx=5, pady=5,sticky="ew")
        ...
        self.entry16.grid(row=4, column=3, padx=5, pady=5,sticky="ew")
        ...
        self.entry17.grid(row=5, column=3, padx=5, pady=5,sticky="ew")
        ...
        self.entry18.grid(row=6, column=3, padx=5, pady=5,sticky="ew")
        ...
        self.net_profit_entry.grid(row=7,column=3,padx=5,pady=5,sticky="ew")
    ...
    def select_info(self):
        ...
        self.entry15.config(state = "normal")
        ...
        if code == 30401:
            if any(data[0]):
                   ...
                   # 작성자 이메일
                    ...
                    # 현 재고
                    self.entry15.insert(0, data[0][13])
                    # 거래 수량
                    self.entry16.insert(0, data[0][14])
                    # 총 가격
                    self.entry17.insert(0, data[0][15])
                    # 부가세
                    self.entry18.insert(0, data[0][16])
                    # 순 이익
                    self.net_profit_entry.insert(0, data[0][17])
                    # 거래처 코드
                    self.correspondent_entry.insert(0, data[0][18])
                    # 사업자 번호
                    self.entry20.insert(0, data[0][19])
                    # 거래처 명
                    self.entry21.insert(0, data[0][20])
                    # 거래처 종류
                    self.entry22.insert(0, data[0][21])
                    # 거래처 주소(국가)
                    self.entry23.insert(0, data[0][22])
                    # 거래처 담당자
                    self.entry24.insert(0, data[0][23])
                    # 거래처 담당자 연락처
                    self.entry25.insert(0, data[0][24])
                    # 거래처 담당자 이메일
                    self.entry26.insert(0, data[0][25])
                    # 납기일
                    self.date_entry2.set_date(data[0][26])
                    # 작성 일자
                    self.date_entry1.set_date(data[0][27])
                    # 수정 일자
                    self.date_entry3.set_date(data[0][28])
            else:
