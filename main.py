...
pyglet.font.add_file("경기천년제목_Medium.ttf")
...
class Main(tk.Tk):
    def __init__(self):
        ...
        # self.font_family = "KOROAD체 Bold"
        # self.font_family = "KOROAD체 Medium"
        # self.font_family = "KOROAD체 Light"
        # self.font_family = "여기어때 잘난체 2 TTF"
        self.font_family = "경기천년제목 Medium"
        ...
        self.lb_logo = tk.Label(self.fr_logo, image=self.im_logo, width=300, height=130, bg=Color.WHITE)
        ...
        self.im_alarm2 = tk.PhotoImage(file="bell-ring.png")
        ...
        self.etc_buttons.append(tk.Button(self.fr_etc_inner, text="e006 테스트", bg=Color.FOCUS, relief="flat", padx=30, pady=5, command=lambda: self.add_nt({
            "from_id": "e001",
            "from_name": "성진하",
            "type": "appr",
            "msg": {
        ...
        self.etc_buttons.append(tk.Button(self.fr_etc_inner, text="e001>e006 결재", bg=Color.FOCUS, relief="flat", padx=30, pady=5, command=self.appr_r))
        ...
        for first in self.category:
            first_tree = Category(tk.Button(self.fr_menu, text=f"　{first}", anchor="w", bg=Color.WHITE, fg=Color.FOCUS, font=self.font(16, "bold"), relief="flat", command=lambda x=first: self.select_category(0, x)))
            self.category_tree.add(first, first_tree)

            for second in self.category[first]:
                second_tree = Category(tk.Button(self.fr_menu, text=f"　　　{second}", anchor="w", bg=Color.WHITE, fg=Color.BLACK, font=self.font(14, "bold"), relief="flat", command=lambda x=second: self.select_category(1, x)))
                first_tree.add(second, second_tree)

                for third in self.category[first][second]:
                    third_tree = Category(tk.Button(self.fr_menu, text=f"　　　　　{third}", anchor="w", bg=Color.WHITE, fg=Color.BLACK, font=self.font(12, "bold"), relief="flat", command=lambda x=third: self.select_category(2, x)))
    ...
    def append_tab(self, first, second, third):
        ...
        tab.button = tk.Button(self.fr_tabs, text=tab.third, height=30, padx=10, font=self.font(10), relief="flat", command=lambda: self.select_tab(tab))
